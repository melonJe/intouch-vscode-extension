...
원본 파일 인코딩은 VS Code의 `files.encoding` 설정(전역/워크스페이스/폴더/`[intouch]` 언어별 오버라이드)을 그대로 사용해서 읽는다. EUC-KR/CP949로 저장된 export가 깨져 보이면 해당 파일 형식에 맞는 인코딩을 `files.encoding`에 설정한 뒤 분할하면 된다. 분할된 결과 파일도 동일한 인코딩으로 저장된다.
...
npm run split -- file1.txt --encoding cp949      # CP949(EUC-KR)로 저장된 파일 읽기, 분할 결과도 CP949로 저장
```
